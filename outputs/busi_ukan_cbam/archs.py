...
class UKAN_CBAM(nn.Module):
    """
    U-KAN with CBAM attention module
    在skip connection处添加CBAM注意力机制，增强特征表达能力
    """
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3, embed_dims=[256, 320, 512], no_kan=False,
    drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[1, 1, 1], **kwargs):
        super().__init__()
        ...
        self.cbam1 = CBAM(kan_input_dim//8)   # for t1
        self.cbam2 = CBAM(kan_input_dim//4)   # for t2
        self.cbam3 = CBAM(kan_input_dim)      # for t3
        self.cbam4 = CBAM(embed_dims[1])      # for t4
    ...
    def forward(self, x):
        ...
        ### Stage 1
        out = F.relu(F.max_pool2d(self.encoder1(x), 2, 2))
        t1 = self.cbam1(out)  # Apply CBAM
        ### Stage 2
        out = F.relu(F.max_pool2d(self.encoder2(out), 2, 2))
        t2 = self.cbam2(out)  # Apply CBAM
        ### Stage 3
        out = F.relu(F.max_pool2d(self.encoder3(out), 2, 2))
        t3 = self.cbam3(out)  # Apply CBAM
